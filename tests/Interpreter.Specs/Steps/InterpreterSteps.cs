...
public class InterpreterSteps
{
    ...
    public void ThenЯПолучаюРезультаты(Table table)
    {
        ...
        IReadOnlyList<decimal> results = fakeEnvironment.GetResults();
        List<decimal> expectedResults = table.Rows
            .Select(row => row.TryGetValue("Результат", out string? valueStr) ? valueStr : null)
            .Where(valueStr => valueStr != null)
            .Select(valueStr => decimal.Parse(valueStr!, CultureInfo.InvariantCulture))
            .ToList();
        ...
        {
            if (Math.Abs(results[i] - expectedResults[i]) > 0.0000000001m)
            {
                throw new Exception(
                    $"Результат [{i}] не совпадает. Ожидалось: {expectedResults[i]}, Получено: {results[i]}");
            }
        }
    }
